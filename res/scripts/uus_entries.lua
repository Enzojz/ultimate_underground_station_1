...
local buildUndergroundEntry = function(config, entryConfig)
    ...
    local arcCoords = pipe.new 
        * {uus.trackGrouping(pipe.new, table.unpack(allArcs))}
    ...
    local laneBuilder = function()
        local function strCoor(l, r, b)
            ...
            return {
                uus.unitLane(ref - vec, b),
                uus.unitLane(ref + vec, ref - vec),
            }
        end
                ...
                return
                    ...
                    * pipe.mapi(function(sizes, i)
                        ...
                        return func.map(sizes.pt, function(p)
                            return uus.unitLane(p, sizes.ac)
                        end)
                    end)
                    +
                    sizes
...
local buildEntry = function(config, entryConfig, retriveRef)
    local allArcs = entryConfig.allArcs
    local gArcs = pipe.new * {uus.trackGrouping(pipe.new, table.unpack(allArcs))}
    ...
    local retriveRef = retriveRef or function()
        ...
        config.entries.main.isLeft 
            and uus.mRot((su.lc[f(su)] - pl.lc[f(pl)]):normalized())
            or uus.mRot((su.rc[f(su)] - pl.rc[f(pl)]):normalized()),
        config.entries.main.isLeft 
    ...
    local laneBuilder = function()
        ...
        local fn = function(p)
            ...
            return pipe.new 
            / uus.unitLane(la.mc[ref.n.l - 2]:avg(la.mc[ref.n.l - 3]), pl.mc[ref.n.p]) 
            / uus.unitLane(la.mc[ref.p.l + 2]:avg(la.mc[ref.p.l + 3]), pl.mc[ref.p.p]) 
            + (p.hasLower
                and {uus.unitLane(la.mc[la.c - 5 - flac]:avg(la.mc[la.c - 4 - flac]), pl.mc[pl.c - 4 - fplc])}
                or
                {})
            + (p.hasUpper
                and {uus.unitLane(la.mc[la.c + 5 + flac]:avg(la.mc[la.c + 4 + flac]), pl.mc[pl.c + 4 + fplc])} or
                {})
            + func.map(
                il(func.range(pl.mc, pl.c - 3, pl.c + 3)),
                function(c) return uus.unitLane(c.i .. coor.transZ(-3.5), c.s .. coor.transZ(-3.5)) end
        )
        end
        ...
        local fn2 = function()
            ...
            local function seperated(p)
                ...
                return pipe.new + ((pl.intersection < (ref.n.l - 2)) and {uus.unitLane(la.mc[ref.n.l - 2]:avg(la.mc[ref.n.l - 3]), pl.mc[ref.n.p])} or {}) +
                    ((pl.intersection < (ref.p.l + 2)) and {uus.unitLane(la.mc[ref.p.l + 2]:avg(la.mc[ref.p.l + 3]), pl.mc[ref.p.p])} or {}) +
                    (p.hasLower and (pl.intersection < (pl.c - 4 - fplc)) and
                    {
                        uus.unitLane(la.mc[la.c - 5 - flac]:avg(la.mc[la.c - 4 - flac]), pl.mc[pl.c - 4 - fplc])
                    } or
                    {})
                    +
            ...
            local combined = function()
                ...
                return pipe.new + ((pl.intersection > (ref.n.l - 2)) and {uus.unitLane(la.mc[ref.n.l - 2]:avg(la.mc[ref.n.l - 3]), pl.mc[ref.n.p])} or {}) +
                    ...
                    ((pl.intersection > pl.c - 3) and (pl.intersection < pl.c + 3) and {
                        uus.unitLane(pl.mc[pl.intersection] .. coor.transZ(-3.5), l.platformO.mc[pl.intersection] .. coor.transZ(-3.5)),
                        uus.unitLane(pl.mc[pl.intersection] .. coor.transZ(-3.5), r.platformO.mc[pl.intersection] .. coor.transZ(-3.5))
                    } or {})
            end
        ...
        return arcCoords
            ...
            * pipe.map(pipe.map(function(pt) return uus.unitLane(pt.r, pt.l) end))
            * pipe.flatten()
    end
    ...
    local accessBuilder = function()
        ...
        return pipe.new *
            func.map(config.buildingParams.platform, function(p) return uus.unitLane(p .. m, cpt) end)
            + func.map(config.buildingParams.entry, function(p) return uus.unitLane(p .. m, coor.xyz(-10, p.y > 0 and 4.5 or -4.5, -0.8) .. mx) end)
            + func.map(config.buildingParams.pass, function(p) return uus.unitLane(p .. m, cupt - coor.xyz(0, 0, 3.5)) end)
